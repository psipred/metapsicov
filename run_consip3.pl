...
while ($masked =~ /([A-Za-z]+)/g)
{
	...
	if (system("$bindir/blastpgp -a 4 -b 0 -v 2000 -j 3 -h 0.001 -e 0.001 -d $maindir/data/blast/unifilt -i $tempdir/$jobid.$roffset.fasta -C $tempdir/$jobid.chk > $tempdir/$jobid.blast") != 0)
	{
	...
	if ($naln_hhblits < 2000)
	{
	    ...
	    if (system("$bindir/jack_hhblits $jobid.$roffset $bindir $tempdir $maindir/data/blast/uniref100 > $tempdir/$jobid.$roffset.jacklog") != 0)
	    {
	...
	if (system("$bindir/psipass2 $maindir/data/psipred/weights_p2.dat 1 1.0 1.0 $tempdir/$jobid.$roffset.ss2 $tempdir/$jobid.ss > /dev/null") != 0)
	{
	...
	if (system("$bindir/alnstats $tempdir/$jobid.$roffset.aln $tempdir/$jobid.$roffset.colstats $tempdir/$jobid.$roffset.pairstats > /dev/null") != 0)
	{
	    &MailError("CONSIP3 ERROR 09 - please report error to psipred\@cs.ucl.ac.uk\n", $email);
	    exit 0;
	}

	$naln = `cat $tempdir/$jobid.$roffset.aln | wc -l`;
	...
	open(S2RFILE, ">$tempdir/$jobid.$roffset.metapsicov.renum.stage2") or die;
	while (<S2FILE>)
	{
	    @recs = split(' ');
	    $recs[0] += $roffset;
	    $recs[1] += $roffset;
	    print S2RFILE "$recs[0] $recs[1] $recs[2] $recs[3] $recs[4]\n";
	}
	close S2FILE;
	close S2RFILE;
...
if ($ndoms)
{
    $results = `cat $tempdir/$jobid.*metapsicov.renum.stage2 | sort -n -r -k 5 | head -5000`;
