...
void            compute_output()
{
    ...
    for (i = NUM_IN + NUM_HID; i < NUM_IN + NUM_HID + NUM_HID2; i++)
    {
	netinp = bias[i];
	tp = weight[i];
	...
	netinp = bias[i];
	tp = weight[i];
	for (j = NUM_IN + NUM_HID; j < NUM_IN + NUM_HID + NUM_HID2; j++)
	    netinp += activation[j] * tp[j];
...
void            load_wts(char *fname)
{
    ...
    /* Load input units to 1st hidden layer weights */
    for (i = NUM_IN; i < NUM_IN + NUM_HID; i++)
	for (j = 0; j < NUM_IN; j++)
	{
    ...
    /* Load hidden units to 2nd hidden layer weights */
    for (i = NUM_IN + NUM_HID; i < NUM_IN + NUM_HID + NUM_HID2; i++)
	for (j = NUM_IN; j < NUM_IN + NUM_HID; j++)
	{
	    if (fscanf(ifp, "%lf", &t) != 1)
...
predict(int argc, char **argv)
{
	...
	for (winpos = 0; winpos < target.length; winpos++)
		...
		for (k = 0, i = CWINL; i <= CWINR; i++)
		    for (j = CWINL; j <= CWINR; j++,k+=6)
			if (winpos + i >= 0 && winpos + i < target.length && winpos2 + j >= 0 && winpos2 + j < target.length)
			{
			    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + k] = target.mimat[winpos + i][winpos2 + j];
			    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + k + 1] = target.mipmat[winpos + i][winpos2 + j];
			    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + k + 2] = target.potmat[winpos + i][winpos2 + j];
			    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + k + 3] = target.psicov[winpos + i][winpos2 + j];
			    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + k + 4] = target.evfold[winpos + i][winpos2 + j];
			    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + k + 5] = target.ccmpred[winpos + i][winpos2 + j];
			}
		
		seqsep = winpos2 - winpos;
		
		if (seqsep < 5)
		    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6] = 1.0;
		else if (seqsep < 14)
		    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 1 + seqsep - 5] = 1.0;
		else if (seqsep < 18)
		    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 10] = 1.0;
		else if (seqsep < 23)
		    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 11] = 1.0;
		else if (seqsep <= 28)
		    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 12] = 1.0;
		else if (seqsep <= 38)
		    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 13] = 1.0;
		else if (seqsep <= 48)
		    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 14] = 1.0;
		else
		    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 15] = 1.0;
		
		for (aa=0; aa<21; aa++)
		    activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 16 + aa] = target.aacomp[aa];
		
		activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 37] = target.cmean;
		activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 38] = target.hmean;
		activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 39] = target.emean;
		activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 40] = target.smean;
		
		activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 41] = log((double)target.length);
		activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 42] = log((double)target.nseq);
		activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 43] = log((double)target.effnseq);
		activation[2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 44] = target.entropmean;
		
#if (2*(WINR-WINL+1) * IPERGRP + (CWINR-CWINL+1) * IPERGRP + (CWINR-CWINL+1) * (CWINR-CWINL+1) * 6 + 45) != NUM_IN
...
main(int argc, char **argv)
{
    ...
    /* malloc_debug(3); */
    if (argc < 9)
	fail("usage : psipred colstats-file pairstats-file psicov-file evfold-file ccmpred-file ss2-file solv-file weight-file1 ... weight-filen");
