/* MetaPSICOV-HB - Neural Network Prediction of Main Chain Hydrogen Bonds */
...
predict(int argc, char **argv)
{
	...
	for (winpos = 0; winpos < target.length; winpos++)
	    for (winpos2 = 0; winpos2 < target.length; winpos2++)
		if (abs(winpos - winpos2) >= 5)
		{
		    for (j = 0; j < NUM_IN; j++)
			activation[j] = 0.0;
...
main(int argc, char **argv)
{
    ...
    /* malloc_debug(3); */
    if (argc < 6)
	fail("usage : metapsicov_hb colstats-file nnpsicov-file ss2-file solv-file weight-file1 ... weight-filen");
